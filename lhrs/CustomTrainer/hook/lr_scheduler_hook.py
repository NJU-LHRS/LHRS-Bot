...
class LrUpdaterHook(HookBase):
    ...
    def __init__(
        ...
        super(LrUpdaterHook, self).__init__()
    ...
    def _set_lr(self, lr_groups):
        if isinstance(self.trainer.optimizer, dict):
            for k, optim in self.trainer.optimizer.items():
                for param_group, lr in zip(optim.param_groups, lr_groups[k]):
                    param_group["lr"] = lr
        else:
            for param_group, lr in zip(self.trainer.optimizer.param_groups, lr_groups):
                param_group["lr"] = lr
    ...
    def get_regular_lr(self):
        if isinstance(self.trainer.optimizer, dict):
            lr_groups = {}
            for k in self.trainer.optimizer.keys():
    ...
    def before_train(self):
        ...
        if isinstance(self.trainer.optimizer, dict):
            ...
            for group in self.trainer.optimizer.param_groups:
                group.setdefault("initial_lr", group["lr"])
            self.base_lr = [
                group["initial_lr"] for group in self.trainer.optimizer.param_groups
            ]
...
class StepLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, base_lr):
        progress = self.trainer.epoch if self.by_epoch else self.trainer.cur_iter
...
class PolyLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, base_lr):
        if self.by_epoch:
            progress = self.trainer.epoch
            max_progress = self.trainer.max_epochs
        else:
            progress = self.trainer.cur_iter
            max_progress = self.trainer.max_iters
...
class FlatCosineAnnealingLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, base_lr):
        if self.by_epoch:
            start = round(self.trainer.max_epochs * self.start_percent)
            progress = self.trainer.epoch - start
            max_progress = self.trainer.max_epochs - start
        else:
            start = round(self.trainer.max_iters * self.start_percent)
            progress = self.trainer.cur_iter - start
            max_progress = self.trainer.max_iters - start
...
class CyclicLrUpdaterHook(LrUpdaterHook):
    ...
    def before_train(self):
        super(CyclicLrUpdaterHook, self).before_train()
        # initiate lr_phases
        # total lr_phases are separated as up and down
        self.max_iter_per_phase = self.trainer.max_iters // self.cyclic_times
    ...
    def get_lr(self, base_lr):
        curr_iter = self.trainer.cur_iter % self.max_iter_per_phase
        curr_cycle = self.trainer.cur_iter // self.max_iter_per_phase
        # Update weight decay
...
class OneCycleLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pct_start < 0 or pct_start > 1 or not isinstance(pct_start, float):
            raise ValueError(
                "expected float between 0 and 1 pct_start, but " f"got {pct_start}"
            )
    ...
    def before_train(self):
        if hasattr(self, "total_steps"):
            ...
            total_steps = self.trainer.max_iters
        if total_steps < self.trainer.max_iters:
            raise ValueError(
                "The total steps must be greater than or equal to max "
                f"iterations {self.trainer.max_iters} of runner, but total steps "
                f"is {total_steps}."
            )
        ...
        if isinstance(self.trainer.optimizer, dict):
            ...
            k = type(self.trainer.optimizer).__name__
            _max_lr = format_param(k, self.trainer.optimizer, self._max_lr)
            self.base_lr = [lr / self.div_factor for lr in _max_lr]
            for group, lr in zip(self.trainer.optimizer.param_groups, self.base_lr):
                group.setdefault("initial_lr", lr)
    ...
    def get_lr(self, base_lr):
        curr_iter = self.trainer.cur_iter
        start_iter = 0
